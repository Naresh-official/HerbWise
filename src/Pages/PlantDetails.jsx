...
export default function PlantDetails() {
	...
	const [plantUses, setPlantUses] = useState(null);
	...
	useEffect(() => {
		if (plantId) {
			fetchPlantDetails(plantId).then(() => {
	...
	useEffect(() => {
		if (plant && !noTokensLeft) {
			getMedicinalUses(); // Call after plant data is fetched
		}
	}, [plant, noTokensLeft]);
	...
	const fetchPlantDetails = async (id, currentTokenIndex = tokenIndex) => {
		...
		try {
			...
			if (
				...
				const nextIndex = parseInt(currentTokenIndex) + 1;
	...
	const getMedicinalUses = async () => {
		setLoading(true);
		try {
			const { data } = await axios.post(
				`${import.meta.env.VITE_GEMINIAI_URL}${
					import.meta.env.VITE_GEMINIAI_API_KEY
				}`,
				{
					contents: [
						{
							parts: [
								{
									text: `Explain medicinal uses of ${plant?.common_name} plant in point wise and in less than 100 words`,
								},
							],
						},
					],
